...
void computeMAC(unsigned char *input, unsigned char *macKey, long numBlocks, unsigned char *MAC){
  ...
  unsigned char *tempDig;
  gcry_md_write(hashHandle, input, (numBlocks * 16));
  tempDig = gcry_md_read(hashHandle, GCRY_MD);
...
long createMessage(unsigned char *aesKey, unsigned char *macKey, unsigned char *cipherText, char *text){
 ...
 unsigned char * blockStr = malloc(blkLength);
  setBlockStr(blockStr, numBlocks);
  charncpy(&cipherText[0], blockStr, blkLength);
	
  unsigned char * sendMAC = malloc(32);  //32 bytes for the MAC we're sending
...
int vrfyMAC(void *input, void *macKey, int blocks, unsigned char *sentMAC){
  ...
  unsigned char *computedMAC = malloc(digLength); 
  computeMAC(input, macKey, blocks, computedMAC);
  if (strncmp((char*)computedMAC, (char*)sentMAC, digLength) == 0) {  //if the first digLenth bytes match (no null term, so strncmp not strcmp)
    vrfy = 1;
  } else {
    vrfy = 0;
  }
...
int deconstructMessage(unsigned char *aesKey, unsigned char *macKey, char **plainTextPtr, unsigned char *input){
  ...
  long numBlocks;
  unsigned char *blockString = malloc(blkLength);
  ...
  unsigned char *MACdStuff = malloc((numBlocks - 2) * blkLength);  //we mac everything but the mac on the end
  charncpy(MACdStuff, input, (numBlocks - 2) * blkLength);
 unsigned char *ctext = malloc((numBlocks - 3) * blkLength);
  //printf("ctext size is: %d\n", (numBlocks - 3) * blkLength);
  charncpy(ctext, (input+blkLength), (numBlocks - 3) * blkLength);

 unsigned char *receivedMAC = malloc(2*blkLength);
...
void hash_pass(char *password, unsigned char *digest){
  ...
  computeMAC(input, (unsigned char*)"000000000000000000000000000042", numBlocks, digest);
}//end hash_pass
...
void AES_CBC_DEC(unsigned char *aesKey, unsigned char *ctext, long numBlocks, char *ptext){ //given the ciphertext with the authentication block already gone
